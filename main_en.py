...
WORDS = ['car', 'house', 'computer', 'python', 'programming', 'window', 'game', 'table', 'keyboard', 'mouse', 'screen', 'internet', 'network', 'software', 'hardware', 'chair'
		 , 'phone', 'apple', 'banana', 'grape', 'orange', 'peach', 'pear', 'plum', 'cherry', 'strawberry', 'watermelon']


class HangmanApp(tk.Tk):
	def __init__(self):
		super().__init__()
		self.title('Hangman - English')
	...
	def _build_ui(self):
		...
		# Új játék gomb
		new_btn = tk.Button(right, text='New Game', command=self.new_game, width=20, bg='#1f4b1f', fg='white', relief='raised')
		new_btn.pack(pady=(10, 5))

		# Nyelv váltás gomb
		switch_btn = tk.Button(right, text='Switch to Hungarian', command=self.switch_language, width=20, bg='#1f4b1f', fg='white', relief='raised')
		switch_btn.pack(pady=(0, 5))

		# Kilépés
		quit_btn = tk.Button(right, text='Exit', command=self.quit, width=20, bg='#1f4b1f', fg='white', relief='raised')
		quit_btn.pack()
	...
	def switch_language(self):
		print("[DEBUG] Switched to Hungarian")
		self.destroy()
		import main_hu
		app = main_hu.HangmanApp()
		app.mainloop()
	...
	def _win(self):
		...
		messagebox.showinfo('You won!', f'Congratulations! Solution: {self.secret}')
		print(f'[DEBUG] WIN')
		for b in self.letter_buttons.values():
			b.config(state='disabled')

	def _lose(self):
		self.message_var.set('You lost!')
		self._update_display()
		messagebox.showinfo('You lost!', f'You lost! Solution: {self.secret}')
		print(f'[DEBUG] LOSE')
		for b in self.letter_buttons.values():
			b.config(state='disabled')
